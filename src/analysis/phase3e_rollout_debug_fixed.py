#!/usr/bin/env python3
import os, sys, copy, numpy as np, torch, torch.optim as optim
...
# Use the simple, guaranteed effect compressor
...
def main():
    ...
    cfg = {
        ...
        'max_grad_norm': 1.0,
        'num_layers': 24,         # controller’s layer slots for debug
        'task_feat_dim': 3,       # minimal task feature dim
        'layer_feat_dim': 2       # minimal layer feature dim
    }
    ...
    # Controller inputs (shapes only; values randomized per sample below)
    task_feat_dim, layer_feat_dim = cfg['task_feat_dim'], cfg['layer_feat_dim']
    ...
    def prep(b): return tok(b['sentence1'], b['sentence2'],
    ...
    for ep in range(cfg['rollout_epochs']):
        ...
        for sidx in range(cfg['rollout_samples']):
            # NEW: randomize task features per sample to diversify actions
            task_feats = torch.randn(1, task_feat_dim, device=device)

            # Forward controller
            r_p, b_p, s_p = mc(task_feats, layer_feats)

            # Temperature softmax over categorical logits
            ...
            # Apply compression and measure L2 delta
        ...
        # Policy update (REINFORCE with entropy bonus)
    ...
    torch.save(mc.state_dict(), os.path.join(cfg['output_dir'], 'final_meta_controller_phase3e_fixed.pt'))
    print('Phase3E debug fixed complete.')
